module Translator (translate, jCreateClass) where
...
translate ::   String 
            ...
            -> Either String (IO ())
translate className destFileName = fmap (translateToJava className destFileName)

translateToJava :: String -> String -> ProgTree LinkId -> IO ()
translateToJava className destFileName tree = do
  let IndentState _ code = execState (jCreateClass className tree) $
        IndentState 0 ""
  writeFile destFileName code
